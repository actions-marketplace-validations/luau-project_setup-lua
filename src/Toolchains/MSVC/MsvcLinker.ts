...
export class MsvcLinker implements ILinker, IWin32ImportLibraryDecorator {
    ...
    addLibDir(dir: string): void {
        this.libDirs.push(
            new ToolchainCompositeArgument(
                [new ToolchainRawArgument(`/LIBPATH:${dir}`)]
            )
        );
    }
        ...
        return new Promise<void>((resolve, reject) => {
            if (this.output) {
                ...
                if (this.outputMode && this.outputMode === "shared") {
                    arrays.push(
                        [new ToolchainCompositeArgument([new ToolchainRawArgument("/DLL")])]
                    );
                }
                if (this.outImplib) {
